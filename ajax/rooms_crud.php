    ...
    if (isset($_GET["fetch_rooms"])) {
      ...
      if (($chk_avail["checkin"] != "") && ($chk_avail["checkout"] != "")) {
        ...
        if ($checkin_date == $checkout_date) {
          echo "<h1 class='text-danger text-center'>Check in and out date  same!</h1>";
          exit;
        }
        else if($checkin_date > $checkout_date) {
          echo "<h1 class='text-danger text-center'>Checkout date is earlier!</h1>";
          exit;
        }
        else if($checkin_date < $today_date){
          echo "<h1 class='text-danger text-center'>Checkin date earlier!</h1>";
          exit;
        }
      }
        ...
        $count_rooms = 0;
        // Output variable that stores room card
        $output = "";
        // Fetching setting table for shutdown value
        ...
        $room_res = select("SELECT * FROM `rooms` WHERE `adult` >= ? AND `children` >= ? AND `status`=? AND `removed`=?",[$adults,$children,1,0],"iiii");
              while($room_data = mysqli_fetch_assoc($room_res)){
                if (($chk_avail["checkin"] != "") && ($chk_avail["checkout"] != "")) {
                 ...
                 // Get facilities for the room with filters
                 $fac_count = 0;
                ...
                $book_btn = "";
                  if (!$settings_r["shutdown"]) {
                    ...
                    $book_btn = "<button onclick='checkLoginToBook($login,$room_data[id])' class='btn btn-sm text-white custome-button shadow-none mb-2 w-100'>Book Now</button>";
                  }
        ...
        if ($count_rooms > 0) {
            ...
            echo "<h1 class='text-danger text-center'>No Rooms Found</h1>";
        }
