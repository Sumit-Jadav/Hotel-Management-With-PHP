<?php
    ...
    if (isset($_POST["get_room"])) {
      $frm_data = filteration($_POST);
      $values = [$frm_data["get_room"]];
      $res1 = select("SELECT * FROM `rooms` WHERE `id`=?",$values,"i");
      $res2 = select("SELECT * FROM `room_features` WHERE `room_id`=?",$values,"i");
      $res3 = select("SELECT * FROM `room_facilities` WHERE `room_id`=?",$values,"i");
    ...
    if (isset($_POST["edit_room"])) {
      ...
      $del_features = deleteq("DELETE FROM `room_features` WHERE `room_id` = ?",[$frm_data["room_id"]],"i");
      $del_facilities = deleteq("DELETE FROM `room_facilities` WHERE `room_id` = ?",[$frm_data["room_id"]],"i");
    ...
    if (isset($_POST["remove_room"])) {
        ...
        $res2 = deleteq("DELETE FROM `room_images` WHERE `room_id` = ?",[$frm_data["room_id"]],"i");
        $res3 = deleteq("DELETE FROM `room_features` WHERE `room_id`=?",[$frm_data["room_id"]],"i");
        $res4 = deleteq("DELETE FROM `room_facilities` WHERE `room_id`=?",[$frm_data["room_id"]],"i");
    ...
    if (isset($_POST["get_room_images"])) {
      ...
      while ($row = mysqli_fetch_assoc($res)) {
        ...
        echo<<<data
          <tr class="align-middle">
            <td><img src='$path$row[image]' width="50%" class="image-fluid"></td>
            <td>$thumb_btn</td>
            <td>
